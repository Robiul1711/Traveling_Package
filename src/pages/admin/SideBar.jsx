    ...
    <>
      ...
      <div
        className={`h-full py-6 ${
          open
            ? "left-0 top-0 w-[320px] z-[220] shadow-lg  overflow-y-auto"
            : "-left-full xl:w-[350px] w-[320px]"
        }
        backdrop-blur-md lg:px-8 px-4 flex flex-col gap-8 border-r shadow-md xlg:static fixed transition-all duration-300`}
      >
        {/* Logo */}
        <Link to={"/"}>
              ...
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-2 rounded-lg text-base font-medium transition-colors duration-200 ${
                  isActive(item?.activePaths)
                    ? "bg-[#6B4F3B] text-[#fff]"
                    : "text-[#000] hover:bg-[#6B4F3B] hover:text-[#ffffff]"
                }`}
              >
                ...
                <div
                  className={`flex items-center justify-between px-4 py-2 cursor-pointer w-full rounded-lg transition-all duration-200 ${
                    parentActive
                      ...
                      : "text-gray-700 hover:bg-[#E3ECFF] hover:text-[#6B4F3B]"
                  }`}
                    ...
                    {item?.sublink?.map((value, subIndex) => (
                      <Link
                        ...
                        className={`block px-4 py-2 rounded-md transition-colors duration-200 ${
                          isActive(item?.activePaths)
                            ? "text-black font-medium bg-[#6B4F3B]"
                            : "text-[#000] font-normal hover:bg-[#6B4F3B]/50"
                        }`}
                        onClick={() => setOpen(false)}
                      >
                        {value?.icon}
